...
def rename_svg_glyph_names(data):
  ...
  svg_path = os.path.join(FONTS_FOLDER_PATH, 'onmsicons.svg')
...
def generate_less(data):
  ...
  variables_file_path = os.path.join(LESS_FOLDER_PATH, '_onmsicons-variables.less')
  icons_file_path = os.path.join(LESS_FOLDER_PATH, '_onmsicons-icons.less')

  d = []
  d.append('/*!');
  d.append('Onmsicons, v%s' % (font_version) );
  d.append('Created by Ben Sperry for the Ionic Framework, http://onmsicons.com/');
  d.append('https://twitter.com/benjsperry  https://twitter.com/ionicframework');
  d.append('MIT License: https://github.com/driftyco/onmsicons');
  d.append('*/');
  d.append('// Onmsicons Variables')
  d.append('// --------------------------\n')
  d.append('@onmsicons-font-path: "../fonts";')
  d.append('@onmsicons-font-family: "%s";' % (font_name) )
  d.append('@onmsicons-version: "%s";' % (font_version) )
  d.append('@onmsicons-prefix: %s;' % (css_prefix) )
  d.append('')
  for ionicon in data['icons']:
  ...
  d = []
  d.append('// Onmsicons Icons')
  d.append('// --------------------------\n')

  group = [ '.%s' % (data['name'].lower()) ]
  for ionicon in data['icons']:
    group.append('.@{onmsicons-prefix}%s:before' % (ionicon['name']) )
  ...
  for ionicon in data['icons']:
    chr_code = ionicon['code'].replace('0x', '\\')
    d.append('.@{onmsicons-prefix}%s:before { content: @ionicon-var-%s; }' % (ionicon['name'], ionicon['name']) )
...
def generate_scss(data):
  ...
  font_version = data['version']
  css_prefix = data['prefix']
  variables_file_path = os.path.join(SCSS_FOLDER_PATH, '_onmsicons-variables.scss')
  icons_file_path = os.path.join(SCSS_FOLDER_PATH, '_onmsicons-icons.scss')

  d = []
  d.append('// Onmsicons Variables')
  d.append('// --------------------------\n')
  d.append('$onmsicons-font-path: "../fonts" !default;')
  d.append('$onmsicons-font-family: "%s" !default;' % (font_name) )
  d.append('$onmsicons-version: "%s" !default;' % (font_version) )
  d.append('$onmsicons-prefix: %s !default;' % (css_prefix) )
  d.append('')
  for ionicon in data['icons']:
    chr_code = ionicon['code'].replace('0x', '\\')
    d.append('$ionicon-var-%s: "%s";' % (ionicon['name'], chr_code) )
...
def generate_css_from_scss(data):
  ...
  scss_file_path = os.path.join(SCSS_FOLDER_PATH, 'onmsicons.scss')
  css_file_path = os.path.join(CSS_FOLDER_PATH, 'onmsicons.css')
  css_min_file_path = os.path.join(CSS_FOLDER_PATH, 'onmsicons.min.css')
...
def generate_composer_json(data):
  ...
  d = {
    "name": "OpenNMS/onmsicons",
    "description": "Custom OpenNMS icons.",
    "keywords": [ "fonts", "icon font", "icons", "ionic", "web font"],
    "homepage": "http://www.opennms.org/",
    "authors": [
      {
        "name": "Benjamin Reed",
        "email": "ranger@opennms.org",
        "role": "Designer",
        "homepage": "https://twitter.com/RangerRick"
      },
      {
        "name": "Adam Bradley",
...
def generate_bower_json(data):
  ...
  d = {
    "name": data['name'],
    "version": data['version'],
    "homepage": "https://github.com/OpenNMS/onmsicons",
    "authors": [
      "Benjamin Reed <ranger@opennms.org>"
    ],
    "description": "Onmsicons",
    "main": [
      "css/%s.css" % (data['name'].lower()),
      "fonts/*"
    ],
    "keywords": [ "fonts", "icon font", "icons", "web font"],
    "license": "MIT",
    "ignore": [
      "**/.*",
      "builder",
